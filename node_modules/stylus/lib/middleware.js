...
function checkImports(path, fn) {
  ...
  nodes.forEach(function(import_1){
    fs.stat(import_1.path, function(err, stat){
      // error
      if (err) {
        --pending || fn(changed);
      // compare mtimes
      } else if (import_1.mtime) {
        if (!changed) changed = stat.mtime > import_1.mtime
          ? import_1.path
          : false;
        import_1.mtime = stat.mtime;
        --pending || fn(changed);
      // first hit, ignore
      } else {
        import_1.mtime = stat.mtime;
